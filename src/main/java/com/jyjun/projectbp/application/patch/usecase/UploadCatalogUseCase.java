...
public class UploadCatalogUseCase {
    ...
    public UploadCatalogOutput execute(UploadCatalogInput input) {
        ...
        updatePatchCatalogService.updateCatalogFileName(input.patchId(), input.catalogFilename());
        Patch updated = updatePatchCatalogService.updateCatalogHashFileName(input.patchId(), input.catalogHashFilename());

        return new UploadCatalogOutput(updated.getId(), updated.getGameId(), updated.getVersion(), updated.getPlatform(), updated.getPatchNote(), updated.getCatalogFileName(), updated.getCatalogHashFileName());
    }
}
