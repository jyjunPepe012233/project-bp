...
@Service
public class CreatePatchUseCase {
    ...
    @Transactional
    public CreatePatchOutput execute(CreatePatchInput input) {
        ...
        if (isRootAccountOfDeveloperUtil.is(currentAccountId, developerId)) {
            ...
            throw new IllegalArgumentException("패치를 등록할 권한이 없습니다.");
        }

        Patch patch = createPatchService.create(input.gameId(), input.version(), input.platform(), input.patchNote());
        return new CreatePatchOutput(patch.getId(), patch.getGameId(), patch.getVersion(), patch.getPlatform(), patch.getPatchNote(), patch.getCreatedAt());
    }
}
